...
def login():
    ...
    if user and data.get("password") == user.password:
        login_user(user)
        return jsonify({'message': "Logged is successfully"})
...
def delete_product(product_id):
    ...
    if product:
        ...
        db.session.commit()
        return jsonify({"messagem": "Product deleted successfully"})
    return jsonify({"messagem": "Product not found"}), 404

# Rota de listagem de produto individual
...
# Rota de remoção de produto do carrinho
...
# Rota de visualização dos produtos do carrinho
